...
const OrdersData = ({ index, data, admin }) => {
  ...
  return (
    <motion.div
      ...
      className="w-full flex flex-col isolate justify-normal px-3 py-2 border relative border-gray-300 bg-cardOverlay drop-shadow-md rounded-md gap-4"
    >
      <div className="w-full flex items-center justify-between">
        <h1 className="text-xl text-headingColor font-semibold ">Orders</h1>

        <div className="flex items-center gap-4">
          <p className="flex icon gap-1  text-textColor ">
            Total : <span className="text-lg text-red-500">₹{data?.total}</span>
          </p>
          ...
          <p
            className={`text-base font-semibold capitalize border border-gray-300 px-2 py-[2px] rounded-sm 
          ${
            (data?.sts === "preparing" && "text-orange-500") ||
            (data?.sts === "cancelled" && "text-red-500") ||
            (data?.sts === "delivered" && "text-emerald-500")
          }`}
          >
            {data?.sts}
          </p>
