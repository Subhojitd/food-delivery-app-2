    ...
    <motion.div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="flex flex-col items-start justify-start gap-6 ">
        ...
        <p className="text-textColor text-lg">
          ...
          culpa voluptates, quae commodi iste harum aperiam dolor sapiente
          deserunt aspernatur explicabo alias modi quia quam labore, repellendus
          ipsum. Suscipit, excepturi? Voluptate sed qui asperiores!
        </p>
        <motion.button
          {...buttonClick}
          ...
          {randomData &&
            randomData.map((data, i) => (
              <motion.div
                key={i}
                {...staggerFadeInOut(i)}
                className="w-32 h-36 md:h-auto md:w-190 p-4 bg-cardOverlay backdrop-blur-md rounded-3xl flex flex-col inset-x-0 justify-center drop-shadow-lg"
              >
                <img
                  src={data.imageURL}
                  className="w-12 h-12 md:w-32 md:h-1/3 md:-mt-16 object-contain
                "
                  alt=""
                />
